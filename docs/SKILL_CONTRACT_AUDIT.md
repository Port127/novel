...
| `data/schemas/chapters.schema.yaml` | `design-chapters` | `golden-chapters`、`paywall-design`、`daily-write`、`data-diagnosis` | 覆盖 `settings/chapters_index.yaml` 的章节列表、状态、摘要、节拍、张力和统计 | 与 `templates/default/settings/chapters_index.yaml` 字段存在漂移；缺少对 `chapter_outlines/` frontmatter 的 dedicated schema。 |
...
| 统一 `chapters.schema.yaml` 与 `templates/default/settings/chapters_index.yaml` | schema/template 对齐 | 两者字段名、统计字段、张力范围不同，容易导致新项目初始化即偏离 schema | P1 | 后续单独设计 |
