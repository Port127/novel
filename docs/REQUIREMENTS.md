...
### 质量引擎 (`src/novel/core/skills/`)

| 引擎 | 功能 |
|------|------|
| `check_logic.py` | 事实核查（角色/时间/地点一致性） |
| `anti_ai_polish.py` | 去 AI 味（五层检测：词汇/句式/段落/叙事/情感） |
| `audit_hooks.py` | 钩子审查（悬念强度/冲突密度/标题吸引力） |
| `design_character.py` | 人设评估（打脸指数/CP感/反派恶心度） |
| `design_paywall.py` | 付费卡点分析 |
| `forge_golden_chapters.py` | 黄金三章结构验证 |
| `scout_topic.py` | 选题分析 |
| `flesh_out_chapter.py` | 章节结构检查 |
| `ask_architect.py` | 节奏分析 |
| `analyze_stats.py` | 统计分析 |

### 工作流引擎 (`src/novel/core/workflow/`)

| 引擎 | 功能 |
|------|------|
| `qa_loop.py` | 质量检查循环 |
| `cost_tracker.py` | 成本追踪 |
| `daily_manager.py` | 日更管理 |
| `manuscript_store.py` | 稿件管理 |
| `truth_parser.py` | 真实解析器 |

### 品类系统 (`src/novel/core/genre/`)

| 模块 | 功能 |
|------|------|
| `profile.py` | 品类画像（GenreProfile + GenreRouter） |

---

## 五、CLI 命令

管理类操作通过 CLI 脚本完成，只做文件系统操作，不调用 LLM：
...
# 生成（代码层，实际创作通过 Skills）
novel generate world <project_id>
novel generate character <project_id>
novel generate outline <project_id>

# 写作（代码层，实际创作通过 Skills）
novel write new <project_id> <chapter_id>
```
