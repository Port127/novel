...
`oh-story-claudecode` 项目提供了成熟的参照。

**路径**：`other/` 是本项目的**同级目录**（非子目录），即 `../other/oh-story-claudecode/`。复用 references/scripts 时使用 `../other/oh-story-claudecode/skills/<skill>/references/` 路径。
...
1. **从 oh-story-claudecode 复用改写**，不从零编写。路径：`../other/oh-story-claudecode/skills/<skill>/references/`
