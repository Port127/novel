...
所有 commit message 遵循 Conventional Commits + 中文正文。
...
- **type**: feat / fix / docs / style / refactor / perf / test / chore / ci / skills
- **scope**: 可选，模块名（如 core、skills、workflow）
- **中文摘要**: ≤50 字符，结尾不加句号

### 强制要求

- 正文必填，禁止只有标题
- "主要改动"必须列出具体内容，禁止"更新代码""修复问题"等笼统描述
- "验证结果"必须记录测试/检查，未运行测试时说明原因
- "背景与目的"和"影响范围"按需省略

### 过滤

自动移除任何 AI 供应商标识（Made-with、Generated by、Created with AI 等）。

### 示例

```
feat(core): 添加 BaseCommercialSkill 基类与成本追踪

主要改动：
- 新增 CommercialVerdict 结构化质量判定类型
- 新增 SkillResult 含 token 消耗和费用估算
- 支持多模型定价表（gpt-4o、gpt-4o-mini、gpt-3.5-turbo）

验证结果：
- pytest tests/core/skills/test_base.py -v，4 passed
```
