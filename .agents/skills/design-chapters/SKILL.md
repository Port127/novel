...
3. 按 `references/chapter-template.md` 提供的 Frontmatter 和 Markdown 格式要求，为每章生成详情，并写入 `settings/chapter_outlines/chapter_{N}.md`。

4. 将每章的基本信息、五要素摘要提取后，保留在内存中供后续 Phase 处理。
5. **启发式提问**：抽取最重要的高潮章节展示其细化契约，提供不同的修改方案 (Option A/B) 供微调，询问是否满足预期。
6. **停顿确认**：与用户多轮迭代，直到本环节内容被用户确认。
...
- **`check-chapters.js`**：检查每章节拍数（3-15）、字数（2000-5000）、必填字段（words_target/density/beats）完整性。
