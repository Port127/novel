...
4. **批处理循环输出（防截断）**：
   ...
   - 每次最多生成 3-5 章的细纲详情，严格按照 `references/chapter-template.md` 提供的 Frontmatter 和 Markdown 格式要求，写入三位补零文件 `settings/chapter_outlines/chapter_XXX.md`（例如 `chapter_001.md`）。
...
- **`check-outlines.js`**：遍历检查所有细纲 Markdown 文件，验证 Frontmatter 合约、结构完整性（必须有特定标题），并对正文中的所有密疏节点字数执行求和计算，确保与目标字数误差不超过 15%。
