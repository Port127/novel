...
> **用途**：读者心理需求、题材生命力规律、跨网站适配、平台差异、书名/简介技法、代入感管理。选题和定位时作为参考库。
...
- 先调用 `/nm` 素材库检索或用户指定样本确认目标平台仍有读者反馈。
...
扫榜规则：1) 参考 `scout-topic` 生成的报告或调用 `/nm` 素材库获取目标平台近期样本 2) 排除自带粉丝基础的头部作品 3) 分析可复用样本的开头结构共性。开头必须在300字内建立核心冲突，一万字定高下。
