...
`settings/notes.yaml` 下的 tracking 节点用于追踪主要角色的状态变化。大纲完成后创建初始状态，每章写完后更新。

### 格式

```yaml
# 角色状态追踪 (在 settings/notes.yaml 中)
# 
...
写作每章前的"状态筛选"步骤从 `settings/notes.yaml` 中提取本章涉及角色的当前状态，作为本节速记的"角色状态"部分。只筛选本章细纲中提到的角色，避免全量读取。
