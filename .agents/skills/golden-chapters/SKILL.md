...
6. 运行 `scripts/check-golden-structure.js` 验证结构
7. 如不通过，调整后重新生成
...
2. 运行 `scripts/check-ai-patterns.js` 检测 AI 模式
3. 运行 `scripts/check-degeneration.js` 检测退化
4. 根据检测结果修改正文
5. 重新运行脚本直到通过
...
2. 最终检查三章连贯性。
3. **写入文件**：生成并写入 `content/chapter_001.md`、`chapter_002.md`、`chapter_003.md` 以及 `golden_chapters_report.yaml`。
