...
- **无商量余地**：绝不允许"因为觉得项目太简单"或"这是一个小功能"而跳过上述 `brainstorming` (设计) 和 `writing-plans` (计划) 流程。新工作必须严格产出到 `docs/superpowers/specs/...` 和 `docs/superpowers/plans/...`；`docs/archive/superpowers/...` 仅用于阶段性归档。
...
├── templates/                 # 项目模板
├── scripts/                   # 全局工具脚本
├── docs/superpowers/          # 当前 TDD Plans 和 Specs 规划文档存放区
