...
| 检查点 | 位置 |
|---|---|
| A. 章节文件 | `chapters/{chapter_id}.md` 是否存在 |
| B. 索引条目 | `chapters/index.yaml` 中是否有 `id: {chapter_id}` |

**处理规则：**

- 两处全不存在 → 正常创建
- 两处全存在 → 提示"章节 {chapter_id} 已存在，是否覆盖？"，等待确认
- **A 不存在但 B 存在**（文件被手动删除，索引残留）→ 报告残留情况，询问用户：
  - 选项 A：清理 `index.yaml` 中的残留条目后重新创建
  - 选项 B：中止，手动检查
- **A 存在但 B 不存在**（索引损坏）→ 报告，询问用户是否重建索引条目
